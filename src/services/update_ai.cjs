...
const p = path.join('c:', 'Users', 'Satyam', 'OneDrive', 'Desktop', 'sanjeevani', 'src', 'services', 'AIEngine.js');
...
code = code.replace(/const apiKey = p1 \+ p2;/, 'const apiKey = p1 + p2;\n\n' + helper);

// We can simplify by just searching for fetch("https://openrouter.ai/api/v1/chat/completions"... and replacing the whole block, but since there are slight differences, we use precise replaces.

code = code.replace(/export const generateFollowUpQuestion = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Follow-up AI Error:", error\);\s*throw new Error\("AI engine failed to generate a response\. Network timeout or rate limit\."\);\s*\}\s*\};/,
...
code = code.replace(/export const processTriage = async \([\s\S]*?catch \(error\) \{\s*console\.error\("OpenRouter API Error:", error\);\s*throw new Error\(error\.message \|\| "Failed to process symptoms with OpenRouter\."\);\s*\}\s*\};/,
...
code = code.replace(/export const runSafetyReviewAgent = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Safety Agent Error:", error\);\s*return \{ isApproved: true, safetyNotes: "Safety Agent offline\. Proceed with standard caution\." \};\s*\}\s*\};/,
...
code = code.replace(/export const runBillingAgent = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Billing Agent Error:", error\);\s*return \{ icd10Code: "UNKNOWN", estimatedCostINR: "Calculation failed", billingNotes: "Billing Agent offline\." \};\s*\}\s*\};/,
...
code = code.replace(/export const runPharmacovigilanceAgent = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Pharmacovigilance Agent Error:", error\);\s*return \{ hasRisk: false, alertMessage: "Pharmacovigilance Agent offline\. Doctor must manually verify allergies\." \};\s*\}\s*\};/,
...
code = code.replace(/export const generateSOAPNote = async \([\s\S]*?catch \(error\) \{\s*console\.error\("AutoScribe Error:", error\);\s*throw new Error\("Failed to generate SOAP note\."\);\s*\}\s*\};/,
...
code = code.replace(/export const checkDrugInteractions = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Pharma AI Error:", error\);\s*throw new Error\("Failed to analyze drug interactions\."\);\s*\}\s*\};/,
...
code = code.replace(/export const runMultiAgentDebate = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Multi-Agent Error:", error\);\s*throw new Error\("Failed to run the Multi-Agent debate\."\);\s*\}\s*\};/,
...
code = code.replace(/export const generateDigitalTwinTrajectory = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Digital Twin Error:", error\);\s*throw new Error\("Failed to generate Digital Twin trajectory\."\);\s*\}\s*\};/,
...
code = code.replace(/export const analyzeGenomicSequence = async \([\s\S]*?catch \(error\) \{\s*console\.error\("Genomics Error:", error\);\s*throw new Error\("Failed to analyze sequence\."\);\s*\}\s*\};/,
...
code = code.replace(/export const analyzeArtTherapyEmotion = async \([\s\S]*?catch \(error\) \{[\s\S]*?\}\s*\};/,
...
console.log('Successfully updated AIEngine.js');
