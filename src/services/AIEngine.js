...
export const runMultiAgentDebate = async (patientCase) => {
  try {
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "meta-llama/llama-3.2-11b-vision-instruct:free", // 100% FREE auto-router, prevents 429 Too Many Requests errors
        max_tokens: 2500,
        messages: [
...
export const generateDigitalTwinTrajectory = async (patientData) => {
  try {
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "meta-llama/llama-3.2-11b-vision-instruct:free", // 100% FREE auto-router model
        messages: [
